...
const toLastNames = (people) => {
  let arr2 = [];
  people.map((item) => {
    return arr2.push(`${item.firstName} ${item.lastName}`);
  });
  return arr2;
};
...
const addPurchases = (arr) => {
  let sum = arr.reduce((result, item) => {
    return result + item.purchasePrice;
  }, 0);
  return sum;
};
...
const countNumberOfElements = (arr) => {
  let sum = arr.reduce((result, item, i) => {
    return i + 1;
  }, 0);
  return sum;
};
...
const reversedString = (str) => {
  ...
  let str2 = arr.reduce((result, item) => {
    return result + item;
  }, "");
  return str2;
};
...
const countNumberOfChildren = (arr) => {
  ...
  let sum = arr2.reduce((result, item) => {
    if (item.children) {
...
const calculateAverage = (arr) => {
  let avg = arr.reduce((result, item, i) => {
    return result + item;
  });
...
const countPrimeNumbers = (arr) => {
  let count = arr.reduce((result, item) => {
    if (isPrime(item)) {
      return result + 1;
    }
    return result;
